...
description: C# desktop test strategy - UI-free core coverage, temp-directory filesystem fixtures, destructive-operation and undo assertions, migration fixtures per schema version, cross-platform CI matrix
...
- UI-free core coverage: rename planning, collision and suffix rules, dedup grouping, filter logic
- Filesystem fixtures in a temp directory: building a tree, asserting what survived an operation, and cleaning up even when the test fails
...
- Path and naming property tests: reserved names, invalid filename characters, long paths, NFC/NFD-divergent filenames
- Migration fixtures: one database fixture per shipped schema version, each opened and migrated forward
- Determinism: no wall clock, no unseeded randomness, no dependence on directory enumeration order
- Cross-platform CI: a Windows-plus-macOS matrix, platform-divergent assertions keyed on `OperatingSystem.IsWindows()`, and tests that pass locally but fail on the other runner
...
| A defect in the code under test - a failing test, or a shipped bug no test caught | `desktop-engineer` owns the triage and fix; this agent owns the suite's side - whether the test was right to catch it, or why none did - and the regression test that pins the fix |
| A unit that cannot be tested without running the app because it is Avalonia-coupled - a plan builder, a scanner, a migration runner | this agent raises it as a testability finding; the project-boundary redesign goes to `desktop-tech-lead` |
...
| A behaviour that cannot be tested because the capability is an OS-level or ecosystem hard block - printing, shell extensions, file associations, Finder Sync | resolve the verdict with `desktop-ecosystem-boundaries`; test the escape hatch that actually ships, and do not author a test against a capability the stack cannot reach |
...
- Use skill: `desktop-core-architecture` for the UI-free core boundary and the injection seams that make a behaviour provable without a window
...
> Push each behaviour to the cheapest layer that can prove it - a plan builder that needs a window to be tested is a design finding, not a testing one. No test touches the user's real filesystem: every filesystem test runs against a temp directory or an injected seam and cleans up even when it fails. A destructive operation is tested by asserting what survived, not by asserting the call returned without throwing, and a test whose result depends on the wall clock, an unseeded RNG, or directory enumeration order is not a test.
