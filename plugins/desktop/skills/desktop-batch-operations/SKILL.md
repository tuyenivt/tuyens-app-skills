...
description: Ship destructive batch ops safely - dry-run preview, undo journal, collision and auto-suffix naming, per-item outcomes, recycle-bin delete.
metadata:
  category: desktop
  tags: [csharp, dotnet, batch, rename, undo, preview, collision, auto-suffix, partial-failure, destructive]
...
- Every item reports its own outcome. A batch never returns one aggregate success for a run containing failures
...
```csharp
public sealed record Plan(IReadOnlyList<Step> Steps, IReadOnlyList<Conflict> Conflicts);
public sealed record Step(string From, string To);

public static Plan BuildPlan(IReadOnlyList<string> inputs, Rules rules);  // pure, no I/O writes
public static Journal Apply(Plan plan);                                   // executes, records
```

`BuildPlan` is pure enough to unit test exhaustively. `Apply` is the only method that touches the disk destructively, which keeps the audit surface small.
...
static string Resolve(string target) =>
    File.Exists(target) ? Suffixed(target, 1) : target;

// Good - the set of names this batch will create participates in the check
static string Resolve(string target, HashSet<string> claimed) {
    var candidate = target;
    for (var n = 1; File.Exists(candidate) || claimed.Contains(candidate); n++)
...
- **Case-insensitive filesystems.** On Windows and default macOS, `Photo.txt` and `photo.txt` are the same name. Build `claimed` with `StringComparer.OrdinalIgnoreCase` where the target filesystem compares that way, or the batch collides at apply time after a clean preview.
- **A case-only rename is not a collision.** `photo.jpg -> Photo.jpg` on a case-insensitive filesystem makes `File.Exists(candidate)` true because the candidate *is* the source. Exempt a step's own source from the exists/claimed check, or every case-only rename gains a spurious suffix.
...
static string Suffixed(string path, int n) {
    var stem = Path.GetFileNameWithoutExtension(path);
    var ext = Path.GetExtension(path);   // splits on the LAST dot - see below
    return Path.Combine(Path.GetDirectoryName(path)!, $"{stem} ({n}){ext}");
}
```
...
- **Multi-part extensions**: `archive.tar.gz` - `GetExtension` returns `.gz` and `GetFileNameWithoutExtension` returns `archive.tar`, so the naive version yields `archive.tar (1).gz`, which is wrong for this case and right for `photo.jpg`. Default: keep a small list of known multi-part extensions (`.tar.gz`, `.tar.bz2`, `.tar.xz`) and suffix before the whole unit - `archive (1).tar.gz`; split on the last extension for everything else. Test whichever rule ships
...
var journal = Journal.FromPlan(plan);
foreach (var step in plan.Steps) File.Move(step.From, step.To);

// Good - records each step as it completes; an interrupted run leaves an accurate journal
var journal = new Journal();
foreach (var step in plan.Steps) {
    try { File.Move(step.From, step.To); journal.Record(new Applied(step.From, step.To)); }
    catch (Exception e) { journal.Record(new Failed(step.From, e.Message)); }
}
```
...
On next launch, a journal left by an interrupted run is surfaced - offer undo of the completed portion or resume of the remainder (resume revalidates the plan first); never silently discard it.

A move that crosses volumes is a copy then a delete, not an atomic rename - perform it as explicit copy and delete, journaled as separate completions, so a crash between them leaves the file in at least one place, never neither.

### Per-item outcomes

```csharp
// Bad - the first exception escapes mid-loop, leaving the tree half-changed
// and the caller uninformed about what was already renamed
public static void Apply(Plan plan) {
    foreach (var step in plan.Steps) File.Move(step.From, step.To);
}

// Good - every item gets an outcome; the caller sees the mix
public static IReadOnlyList<StepOutcome> Apply(Plan plan);

public abstract record StepOutcome;
public sealed record Applied(string From, string To) : StepOutcome;
public sealed record Skipped(string Path, SkipReason Reason) : StepOutcome;
public sealed record Failed(string Path, OpError Error) : StepOutcome;
```

The throwing version is the more common defect and the worse one: it stops at the first permission error, having already renamed some files, and surfaces an exception that says nothing about what was done.

### Delete is not rename

For delete specifically, prefer the OS recycle bin over `File.Delete`. It gives the user an undo path the app does not have to implement, and it is what they expect from a desktop tool. The BCL has no cross-platform API for it - the shell interop (`IFileOperation` on Windows, `NSFileManager` trash on macOS) belongs to `desktop-platform-integration`. Where a permanent delete is genuinely wanted, make it an explicit choice, not the default.
...
`[Must]` when the consequence is lost, overwritten, or unrecoverable user data - most findings here are. `[Recommend]` when the defect costs clarity or convention (suffix wording, report formatting) without risking data. One block per `Gap` value: distinct gaps at one location stay separate blocks; the same gap repeated across a file merges into one block with every location named.
...
- Suffixing a case-only rename because the candidate matched its own source
- Ignoring the rename-cycle case where a step's source is an earlier step's target
- Appending `(1)` after the extension
- Letting the first exception escape the apply loop and abort the batch
- Returning one aggregate success for a run that had failures
...
- `File.Delete` as the default delete where the recycle bin is available
