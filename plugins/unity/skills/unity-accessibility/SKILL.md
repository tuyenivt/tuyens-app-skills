...
`Evidence: inferred` is required whenever the source was not read, and caps the block at High - write the capped severity in the header, not the uncapped one, and name the uncapped band in `Impact`. `Evidence` records whether the source was read; `Verified` below records whether the behaviour was exercised on a device.
