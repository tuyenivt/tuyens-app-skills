...
When reviewing, emit one block per defect, highest severity first. One line tripping several categories is one block under the category that carries the fix; name the others in `Impact`.
